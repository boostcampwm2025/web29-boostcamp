  ...
  private validateEc2Requirements(
    ...
    for (const [ec2Name, req] of Object.entries(reqs ?? {})) {
      ...
      const targetInstances =
        ec2Name === '*'
          ? refinedInstances
          : refinedInstances.filter((i) => i.name === ec2Name);

      for (const instance of targetInstances) {
        const displayName = instance.name || ec2Name;
        ...
        if (req.expectedSubnet) {
          if (instance.subnetName !== req.expectedSubnet) {
            feedbacks.push({
              ...
              message: `EC2 인스턴스 ${displayName}이(가) 잘못된 서브넷(${instance.subnetName})에 배치되었습니다. 요구사항: ${req.expectedSubnet}`,
            });
          }
        }
        ...
        if (req.requirePublicIp) {
          ...
          if (!instance.autoAssignPublicIp) {
            feedbacks.push({
              ...
              message: `EC2 인스턴스 ${displayName}에 퍼블릭 IP 자동 할당 설정이 누락되었습니다.`,
            });
          }
        }
        ...
        if (req.expectedOsType) {
          if (instance.osType !== req.expectedOsType) {
            feedbacks.push({
              ...
              message: `EC2 인스턴스 ${displayName}의 OS 이미지가 올바르지 않습니다. (현재: ${instance.osType}, 요구: ${req.expectedOsType})`,
            });
          }
        }
        ...
        if (req.expectedInstanceType) {
          if (instance.instanceType !== req.expectedInstanceType) {
            feedbacks.push({
              ...
              message: `EC2 인스턴스 ${displayName}의 인스턴스 타입이 올바르지 않습니다. (현재: ${instance.instanceType}, 요구: ${req.expectedInstanceType})`,
            });
          }
        }
        ...
        if (req.requireUserData && !instance.userData?.trim()) {
          feedbacks.push({
            ...
            message: `EC2 인스턴스 ${displayName}에 User Data 스크립트가 필요합니다.`,
          });
        }
